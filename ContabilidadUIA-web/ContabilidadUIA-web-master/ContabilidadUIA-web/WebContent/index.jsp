...
<body>	
	...
	<form name="formCuentas" id="form2" action="/cuentas.jsp">
  		...
  		<c:forEach var="cuentas" items="${contabilidad.getListaProveedores()}">
	...
	<form name="formCheques" id="form2" action="/cuentas.jsp">
  		Cheques: <select name="cheques">	
  		<c:forEach var="cheques" items="${contabilidad.getListaProveedores()}">
			<option value="${cheques.id}">${cheques.name}</option>	
		</c:forEach>
		</select>
  		<input type="submit" value="Submit">
  	</form>
